...
function animatedRect() {
    ...
    let rect = new Elem(svg, 'rect')
        ...
        .attr('width', 120).attr('height', 80)
        .attr('fill', '#95B3D7');
    let click = Observable.fromEvent(svg, 'click');
...
function drawRectsObservable() {
    const svg = document.getElementById("drawRects"), mousedown = Observable.fromEvent(svg, 'mousedown')
        .map(event => ({ e: event, svgRect: svg.getBoundingClientRect() }))
        .map(({ e, svgRect }) => ({ rect: new Elem(svg, 'rect')
            .attr('x', String(e.clientX - svgRect.left))
            .attr('y', String(e.clientY - svgRect.top))
            .attr('width', '5')
            .attr('height', '5')
            .attr('fill', '#95B3D7'), svgRect }))
        .subscribe(({ rect, svgRect }) => {
        const x1 = Number(rect.attr('x')), y1 = Number(rect.attr('y'));
        Observable.fromEvent(svg, 'mousemove')
            .takeUntil(Observable.fromEvent(svg, 'mouseup'))
            .map(({ clientX, clientY }) => ({
            rect, x1, y1, x2: clientX - svgRect.left, y2: clientY - svgRect.top
        }))
            .subscribe(({ rect, x1, y1, x2, y2 }) => rect.attr('x', String(Math.min(x1, x2)))
            .attr('y', String(Math.min(y1, y2)))
            .attr('width', String(Math.abs(x1 - x2)))
            .attr('height', String(Math.abs(y1 - y2))));
    });
}
function drawAndDragRectsObservable() {
    const svg = document.getElementById("drawAndDragRects"), mousemove = Observable.fromEvent(svg, 'mousemove'), mouseup = Observable.fromEvent(svg, 'mouseup'), mousedown = Observable.fromEvent(svg, 'mousedown')
        ...
        Observable.fromEvent(svg, 'mousemove')
            ...
            .attr('width', String(Math.abs(x1 - x2)))
            .attr('height', String(Math.abs(y1 - y2))));
        rect.observe('mousedown').subscribe((e) => e.stopPropagation());
        rect.observe('mousedown')
            .map(({ clientX, clientY }) => ({ xOffset: Number(rect.attr('x')) - clientX,
            yOffset: Number(rect.attr('y')) - clientY }))
            .flatMap(({ xOffset, yOffset }) => mousemove
