/**
 * I have added one takeWhile function into Observable class
 * takeWhile function emits values as long as each value satisfies the given predicate, then completes and unsubscribes as soon as this predicate is not satisfied.
 * This function takes a function which returns a boolean (type U) value as parameter, and returns Observable of notification until predicate returns false
 * It is used to unsubscribe the whole game event when some conditions are fulfilled (i.e. score = 11)
 * I also converted the type into generic types
*/
...
class Observable<T> {
   ...
   */
  map<R>(transform: (_:T)=>R): Observable<R> {
   ...
   */
  filter<U>(condition: (_:T)=>U): Observable<T> {
      // Your code here ...
      return new Observable<T>(observer => 
        this.subscribe(e=>{
   ...
   */
  flatMap<W>(streamCreator: (_: T) => Observable<W>): Observable<W> {
    return new Observable<W>((observer: Observer<W>)=>{
  ...
  /**
   * creates a child Observable that is detached when the given Observable fires
   * @param predicate function whose notification will complete this Observable when it returns true
   * @return child Observable of notifications up until predicate returns false
   */
  takeWhile<U>(predicate:(_:T) => U): Observable<T> {
    return new Observable<T>(observer =>
        this.subscribe(e=> {if (predicate(e)) observer.next(e)}, 
        () => {
          const oUnsub = this.subscribe(_=>{
            observer.complete();
            oUnsub;})
        observer.complete();
        oUnsub(); 
